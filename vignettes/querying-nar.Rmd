...
`Streets` are gazetteers `normalize_address()` builds at import. `RqaAddresses`
and `RqaStreets` appear only if `rqa_import()` has been run — they are Quebec's
own register, kept beside NAR rather than merged into it, and
`vignette("geocoding")` covers what they are for.
