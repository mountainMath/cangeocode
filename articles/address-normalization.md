...
`O`. So canonicalization is conditioned on the province, and the
province you supply through `known` materially changes the answer:

``` r

normalize_address(rep("100 Main Avenue West, Springfield", 2),
                  known = list(PROV_ABVN = c("ON", "QC"))) |>
...
## When the address was never one string

The province is one of ten components `known` accepts, and the rest work
the same way. Address data rarely arrives as a single line: an
assessment roll carries the community in a column, a filing carries the
postal code, a cleaned list carries everything but the street. `known`
is how that structure reaches the parser instead of being concatenated
into a string and re-derived from it.

``` r

normalize_address(c("100 Main Avenue West", "12 Rue Notre-Dame Est"),
                  known = list(MUN_NAME = c("Springfield", "Montréal"),
                               PROV_ABVN = c("ON", "QC"))) |>
  select(CIVIC_NO, STREET_NAME, STREET_TYPE, STREET_DIR, MUN_NAME, PROV_ABVN)
#> # A tibble: 2 × 6
#>   CIVIC_NO STREET_NAME STREET_TYPE STREET_DIR MUN_NAME    PROV_ABVN
#>      <dbl> <chr>       <chr>       <chr>      <chr>       <chr>    
#> 1      100 MAIN        AVE         W          SPRINGFIELD ON       
#> 2       12 NOTRE-DAME  RUE         E          MONTRÉAL    QC
```

Every component is authoritative: it overrides whatever the string
parsed to, it constrains the gazetteer’s search, and it comes back out
on the row. `NA` for a row means nothing is known about that row, so the
parse stands.

Two of the ten are the same word for different things. `MUN_NAME` is the
**mailing city**, the name on the envelope. `CSD_NAME` is the **census
subdivision**, the administrative unit, and it resolves through NAR’s
alias set — so `CSD_NAME = "Toronto"` reaches an address filed under
`SCARBOROUGH` and `MUN_NAME = "Toronto"` does not. The two do not nest,
so they are separate components rather than one with a mode;
...
A third value, `rqa`, appears in Quebec once
[`rqa_import()`](https://mountainmath.github.io/cangeocode/reference/rqa_import.md)
has been run: the street was found in the *Répertoire québécois des
adresses* rather than in NAR. It is a real confirmation, and it is
deliberately not called `gazetteer`, because a row carrying it will
still not join `Addresses` — NAR does not hold that address.
...
`mun_remapped` is the fourth, and it answers a question the other three
cannot: whether the municipality you get back is the one you wrote. The
gazetteer resolves a community name through the census subdivision it
belongs to, so a street can be confirmed — `parse_source` of
`gazetteer`, high `confidence` — in a different community of the same
regional municipality. That is usually the feature working, since rural
communities are filed under the city that delivers their mail. It is
worth knowing about either way:

``` r

normalize_address(c("2 Barrington St, Halifax NS", "25 River Rd, Moser River NS",
                    "36 Lakeview Dr, Howie Centre NS"), con = con) |>
  select(STREET_NAME, MUN_NAME, mun_remapped, mun_evidence, confidence)
#> # A tibble: 3 × 5
#>   STREET_NAME MUN_NAME         mun_remapped mun_evidence confidence
#>   <chr>       <chr>            <lgl>        <chr>             <dbl>
#> 1 Barrington  HALIFAX          FALSE        kept               0.88
#> 2 River       HALIFAX          TRUE         copostal           1   
#> 3 Lakeview    CONQUERALL MILLS TRUE         untestable         0.9
```

Note that the remapped rows are not the low-confidence ones.
`confidence` grades how well the *street* matched, and Moser River’s
River Rd matched perfectly — under a community name 118 km from the one
that was written. That is why this is a column and not a confidence
penalty.

`TRUE` also covers the case where the string named no municipality at
all and the gazetteer determined one; `NA` means there is still none. If
you are matching against a list keyed on the municipality, this is the
column that tells you which keys were rewritten under you.

`mun_evidence` sits beside it and says *why* the substitution was
allowed to stand. Three of its values are attestations — `kept` (nothing
was substituted), `copostal` (the two names share a full postal code in
the register, so they are two labels for one delivery geography) and
`csd` (the name written is the census subdivision the street sits in,
which is how amalgamations and legacy names arrive: `Toronto` for a
street the register still mails to `North York`). The other three are
not: `unattested` was checked and corroborated by nothing, `untestable`
names a community the register files no postal-coded mail under so there
was nothing to check against, and `inferred` means you named no
municipality and one was determined for you. An unattested substitution
is fined during scoring and has to be an exact or near-exact street name
agreeing on everything else to survive;
[`geocode()`](https://mountainmath.github.io/cangeocode/reference/geocode.md)
prices what does survive into `uncertainty_m`. None of this is a
guarantee — the Moser River row is attested and 118 km out — but it is
...
**Quebec is measured against a register NAR does not fully carry.** NAR
is missing about 475,000 Quebec addresses that the province’s own
register holds, so judging a Quebec parse against NAR alone scores real
addresses as failures. Of the 942 Quebec filings in the sample, 77.5%
resolve to a NAR address and 83.0% to one in NAR or the *Répertoire*.
Running
[`rqa_import()`](https://mountainmath.github.io/cangeocode/reference/rqa_import.md)
is what buys that second judge. What it does *not* buy is much of a
better parse: the `rqa` gazetteer pass answered 4 of those 942 rows,
because the Quebec filings NAR cannot settle are overwhelmingly mistyped
rather than uncovered — `Bouceherville`, `ST.CATHERINE ST.WEST`,
`1603 - 3410, rue Peel`. On the addresses the register holds and NAR
does not, the same pass answers 8.9%.
