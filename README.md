...
`cangeocode` is designed to provide open **geocoding**, **reverse geocoding**, and **address normalization** for Canadian addresses built on data under a permissive license. 
At the core sits Statistics Canada's
[National Address Repository](https://www.statcan.gc.ca/en/lode/databases/nar)
(NAR), a national list of structured civic addresses with coordinates. While a solid base, the NAR on it's own can't provide robust geocoding. Some records in the NAR lack spatial coordinates, some are sparially mis-located, and a range of addresses are missing entirely. `cangeocode` provides a multi-tiered approach to geocoding that uses NAR as the base, and adds additional sources to fill in the gaps.

Two important additions are Quebec's
[Répertoire québécois des adresses](https://www.donneesquebec.ca/recherche/dataset/adresses-quebec) which fills some impotant coverage gaps of the NAR, and Statistics Canada's
[Road Network File](https://www150.statcan.gc.ca/n1/en/catalogue/92-500-X), which
is what reaches streets NAR does not carry.

The data is imported into a local [DuckDB](https://duckdb.org) database for fast, offline access. The package provides a set of functions to query the database and geocode addresses, as well as a set of functions to normalize addresses and parse them into their components.
...
`osm_geocode()` calls the OpenStreetMap geocoder NRCan hosts. Nothing contacts any of them unless you ask for it.

## What you may do with the results

The package is MIT, but the coordinates it hands back are not the package's to
licence — they belong to the sources it draws on. A default `geocode()` call
mixes NAR (Statistics Canada Open Licence), Quebec's register (CC-BY 4.0) and
the road network file (Statistics Canada Open Licence); adding `"bc"`, `"qc"` or
`"nrcan"` brings in the Open Government Licence of British Columbia or of
Canada. **Every one of those is an attribution licence, and they compose**: you
may use, publish and sell work built on `geocode()` output, commercially
included, provided each source you actually drew on is credited. Nothing else is
asked of you — no share-alike, no fee, no key. `match_method` tells you which
tier answered each row, so you can see which credits you owe; the CC-BY one is
returned ready-made by `rqa_attribution()`.

The one exception is `osm_geocode()`, whose OpenStreetMap data is **ODbL** —
attribution *plus* share-alike, with the obligation attaching to a derived
database. That is why it is not a `geocode()` tier: no default call can
quietly change what you may do with your whole result table.
...
| [`source-rqa`](https://mountainmath.github.io/cangeocode/articles/source-rqa.html) | Répertoire québécois des adresses | offline `"rqa"` tier, via `rqa_import()` | CC-BY 4.0 |
| [`source-rnf`](https://mountainmath.github.io/cangeocode/articles/source-rnf.html) | Statistics Canada Road Network File | offline `"rnf"` tier, via `rnf_import()` | StatCan Open Licence |
| [`source-bc`](https://mountainmath.github.io/cangeocode/articles/source-bc.html) | BC Address Geocoder | online tier, and `bc_validate()` | OGL – British Columbia |
...
| [`source-qc`](https://mountainmath.github.io/cangeocode/articles/source-qc.html) | Quebec MRNF geocoder | online tier, `qc_validate()`, `qc_reverse_geocode()` | CC-BY 4.0 |
| [`source-osm`](https://mountainmath.github.io/cangeocode/articles/source-osm.html) | NRCan-hosted Nominatim | bound as `osm_geocode()`, deliberately not a tier | **ODbL 1.0** |

The last column is why the last row is not a tier: the first six are attribution
licences and compose with each other, ODbL adds share-alike and does not. See
[what you may do with the results](#what-you-may-do-with-the-results).
...
`known` is what you already know about the address, in the same column names
the output uses: `CIVIC_NO`, `STREET_NAME`, `STREET_TYPE`, `STREET_DIR`,
`APT_NO_LABEL`, `CIVIC_NO_SUFFIX`, `MUN_NAME`, `CSD_NAME`, `PROV_ABVN`,
`POSTAL_CODE`. Every component is **authoritative**: it overrides whatever the
address string said, it constrains the search, and it lands on the returned row
too. So does `within`.
...
The two municipality components are two different questions. `MUN_NAME` is the
**mailing city**, compared straight at the name on the envelope. `CSD_NAME` is
the **census subdivision**, resolved through NAR's alias set, so
`CSD_NAME = "Toronto"` finds the addresses NAR files under `SCARBOROUGH`,
`ETOBICOKE` and `NORTH YORK` and `MUN_NAME = "Toronto"` does not. Supply both
to narrow to one community inside an amalgamated city.
...
`normalize_address()` takes the same `known` list as `geocode()`, which is how
an address that was never one string — a roll with the community and the postal
code in columns of their own — reaches the parser with its structure intact.

**Canonical forms are province-conditioned.** NAR writes `AVE` in Ontario and
`AV` in Quebec, `BLVD` against `BOUL`, `W` against `O`, so
`known$PROV_ABVN` is not a hint but a choice of vocabulary. Pass it if your data
has it.
...
instance Natural Resources Canada hosts on the Federal Geospatial Platform — not the volunteer-funded `nominatim.openstreetmap.org`, whose usage policy forbids bulk geocoding. It needs no API key and no local database.
...
source in this package is under an attribution-only open licence. Folding a few ODbL
